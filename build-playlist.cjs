...
const MUSIC_BASE_URL = process.env.MUSIC_BASE_URL ? process.env.MUSIC_BASE_URL.replace(/\/$/, '') : null;
...
async function scanDir(dir, catalogue) {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    const relativePath = `music/${catalogue}/${f}`;
    const file = MUSIC_BASE_URL
      ? `${MUSIC_BASE_URL}/music/${encodeURIComponent(catalogue)}/${encodeURIComponent(f)}`
      : relativePath;
    let duration = null;
    if (musicMetadata) {
      try {
        ...
        if (meta.format && typeof meta.format.duration === 'number') duration = meta.format.duration;
      } catch (_) {}
    }
    tracks.push({
      file,
...
const catalogues = ['80s', '90s', '2000', 'MrMelo Mix', 'Oldies and Goldies', 'Summer Dance', 'FootFans'];

(async () => {
  let all = [];
  for (const cat of catalogues) {
    const dir = path.join(MUSIC_BASE, cat);
    const tracks = await scanDir(dir, cat);
